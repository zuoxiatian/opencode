...
describe("authorized browser capability policy", () => {
  ...
  test("uses webfetch for navigation and no permission for generic HTML reads", async () => {
    ...
    await Effect.runPromise(authorizeBrowserCommand(
      { command: "tab.state", tabId: "tab-test" },
      html.ctx,
      async () => {
  ...
  test("uses browser_origin and browser_cdp for CDP without exposing its expression", async () => {
    const cdp = context()
    ...
    expect(cdp.requests.map((request) => request.permission)).toEqual([
      "browser_origin",
      "browser_cdp",
    ])
  ...
  test("requires origin authorization for automation and interaction authorization for actions", async () => {
    const snapshot = context()
    const snapshotOrigin = await Effect.runPromise(authorizeBrowserCommand(
    ...
    expect(snapshot.requests.map((request) => request.permission)).toEqual(["browser_origin"])
    expect(click.requests.map((request) => request.permission)).toEqual([
      "browser_origin",
      "browser_interaction",
    ])
  })
